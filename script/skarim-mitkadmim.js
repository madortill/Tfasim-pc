...
function start(event) {
    ...
    $(".next-btn").show();
    $(".help").show();
    $(".girl").css({backgroundImage: 'url("assets/images/pcidle.svg")'});
    $(".next-btn").on("click", function() {
        $(".explanation").hide();
        $(".next-btn").hide();
        document.querySelector(".marking").classList.remove("inactive");
        $(".bottom").show();
        $(".girl").css({backgroundImage: 'url("assets/images/pctalk.svg")'});
            setFirstScreen();
            setMidSummery();
            set101();
    });
...
function goToTofes(e) {
    ...
    document.querySelectorAll("body > .screen")[2].classList.remove("inactive");
        document.querySelector(".tofes").style.display = "flex";
    $(".girl").hide();
}
...
function finishLomda(correctCounter) {
    if (correctCounter == draggArr.length) {
        ...
        document.querySelector(".tofes").style.display = "none";
        $(".girl").show();
        $(".girl").css({backgroundImage: 'url("assets/images/pcend.svg")'});
        console.log("finish");
    }
